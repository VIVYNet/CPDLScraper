...
def scrape(link):
    ...
    try:        
        ...
        wikitext_soup = BeautifulSoup(urlopen(f"https://www.cpdl.org/wiki/index.php?title={title}&action=edit"), "lxml")
        ...
        indexes = [str(item.title).lower() for item in general_wikitext.sections]
        ...
        wikitext_raw = str(general_wikitext.sections[indexes.index("general information")])
        ...
        for index, item in enumerate(templates):
            ...
            # print(item)
            # print(f'{index} {item.name} - {", ".join([information_parse(str(i)) for i in item.params])}')
            # print()
        ...
        try:
            # Get wikitext data from given page
            wikitext_raw = str(general_wikitext.sections[indexes.index("original text and translations")])
        
        # Except
        except:
            # Get wikitext data from given page
            wikitext_raw = str(general_wikitext.sections[indexes.index("text and translations")])
        ...
        for index, item in enumerate(templates):
            ...
            if item.name.lower() in TR_KEYS:
                ...
                translations[str(item.name.lower())] = [str(i) for i in item.params]
